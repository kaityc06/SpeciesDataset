...
def _done_markers(dataset_dir: str, total_shards: int) -> list[str]:
    return [
        os.path.join(dataset_dir, f"masks_{i:05d}_of_{total_shards:05d}.parquet.done")
...
def _patched_merge_shards(dataset_dir: str, output_path: str = None, delete_shards: bool = False) -> None:
    ...
    if delete_shards and total_shards is not None:
        if len(shard_files) < total_shards:
            ...
            return
        done_markers = _done_markers(dataset_dir, total_shards)
        if not all(os.path.exists(path) for path in done_markers):
            done_count = sum(os.path.exists(path) for path in done_markers)
            print(f"[{DATASET_LABEL}] Waiting to merge: {done_count}/{total_shards} done markers exist")
            return
